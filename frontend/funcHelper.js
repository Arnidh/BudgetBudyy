export const calculateDateDiff = (createdAt, currentDate) => {
  ...
  if (dd !== 0) {
    if (dd < 7) {
      return dd === 1 ? dd + " day ago" : dd + " days ago";
    } else {
      return "so long ago";
    }
  } else if (hh !== 0) {
    return hh === 1 ? hh + " hour ago" : hh + " hours ago";
  } else if (mm !== 0) {
    return mm === 1 ? mm + " minute ago" : mm + " minutes ago";
  } else if (ss !== 0) {
    return ss === 1 ? ss + " second ago" : ss + " seconds ago";
  } else {
    return "Just now";
  }
};
